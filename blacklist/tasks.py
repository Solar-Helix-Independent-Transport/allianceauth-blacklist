...
def run_blacklist_update(note_id):
    ...
    if instance.eve_catagory == "character":
        ...
        exists = blk_state.member_characters.filter(
            character_id=instance.eve_id).exists()
        if blacklist and exists:
            logger.debug(f"'{instance.eve_name}'' is already Blacklisted")
        elif blacklist and not exists:
            if eve_char is None:
                logger.debug(
                    f"Creating new Auth Model for '{instance.eve_name}'")
                eve_char = EveCharacter.objects.create_character(
                    instance.eve_id)
    ...
    if instance.eve_catagory == "corporation":
        logger.debug(
            f"Checking Corporation for blacklist '{instance.eve_name}'")
        eve_corp = EveCorporationInfo.objects.filter(
            corporation_id=instance.eve_id)
        blacklist = instance.blacklisted
        exists = blk_state.member_corporations.filter(
            corporation_id=instance.eve_id).exists()
        if blacklist and exists:
            logger.debug(f"'{instance.eve_name}'' is already Blacklisted")
        elif blacklist and not exists:
    ...
    if instance.eve_catagory == "alliance":
        ...
        blacklist = instance.blacklisted
        exists = blk_state.member_alliances.filter(
            alliance_id=instance.eve_id).exists()
        if blacklist and exists:
            logger.debug(f"'{instance.eve_name}'' is already Blacklisted")
        elif blacklist and not exists:
            if eve_alli.exists() is False:
                logger.debug(
                    f"Creating new Auth Model for '{instance.eve_name}'")
                eve_alli = EveAllianceInfo.objects.create_alliance(
                    instance.eve_id)
            else:
