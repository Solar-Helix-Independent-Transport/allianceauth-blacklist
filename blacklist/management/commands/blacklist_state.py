...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            "Creating/Reseting/Fixing the Blacklist State for the Blacklist Module")
        blacklist_name = app_settings.BLACKLIST_STATE_NAME
        max_prio = State.objects.all().exclude(name=blacklist_name).aggregate(
            max_prio=Max('priority'))["max_prio"]
