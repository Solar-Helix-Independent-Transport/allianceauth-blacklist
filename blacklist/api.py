...
def datatables_server_notes(request, params: Query[DatatablesServerNotes]):
    ...
    limit = start + length
    # The dict wont cast, use manual GET
    search_string = request.GET['search[value]']
    try:
        # The dict wont cast, use manual GET
        order_col = int(request.GET['order[0][column]'])
    except Exception:
        order_col = 0
    # The dict wont cast, use manual GET
    ...
    datatables_data['recordsFiltered'] = EveNote.objects.filter(
        filter_q).count()
...
@api.get("datatables_server_blacklist/", tags=["Datatables"])
def datatables_server_blacklist(request, params: Query[DatatablesServerNotes]):
    # BEGIN ServerSide Datatables _template_
    length = int(params.length)
    start = int(params.start)
    limit = start + length
    ...
    items = []
    for note in EveNote.objects.filter(filter_q).order_by(order).filter(blacklisted=True)[start:limit]:
        ctx = {"note": note}
        row = []
        for t in templates:
            if "{{" in t:
    ...
    datatables_data = {}
    datatables_data['draw'] = draw
    datatables_data['recordsTotal'] = EveNote.objects.filter(
        blacklisted=True).count()
    datatables_data['recordsFiltered'] = EveNote.objects.filter(blacklisted=True).filter(
        filter_q).count()
    datatables_data['data'] = items
